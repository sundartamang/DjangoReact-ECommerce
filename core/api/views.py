...
class OrderQuantityUpdateView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if order_qs.exists():
            ...
            return Response({"message":"you do not have an active item"},status = HTTP_400_BAD_REQUEST)
...
class AddToCartView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        variations = self.request.data.get('variations', [None])
        print(variations)
        if slug is None:
        ...
        #This for loop chek if there
        #  is any same order item or not (whcih is already ordered) like [1,4]
        for v in variations: #[1,4]
            order_item_qs = order_item_qs.filter(
                item_variations__exact = v
            )

        #if order item exist then just increse the item number
...
class OrderDetailView(RetrieveAPIView):
    ...
    def get_object(self):
        try:
            ...
            # return Response({"message","you do not have an active"},status=HTTP_400_BAD_REQUEST)
